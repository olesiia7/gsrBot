...
public class AddLogCommand extends BotCommand {
    ...
    public AddLogCommand(DbController db, EventManager eventManager) {
    ...
    private void enterDate(Category category, String description) throws TelegramApiException {
        ...
        bot.setListener(answer -> {
            try {
                Date date = toDate((String) answer);
    ...
    private static Date toDate(String inputDate) throws DateTimeParseException {
        LocalDate localDate = LocalDate.parse(inputDate, DATE_FORMATTER);
        return Date.valueOf(localDate);
    }

    //todo: вынести в отдельный класс методы преобразования даты и цены
